...
| 경로 | 내용 |
| --- | --- |
| `/mix` | 믹스 의뢰 — 앨범 그리드, Before/After 플레이어, 가격 3종, 후기 |
| `/posts` | 글 — 블로그와 평론을 합친 통합 게시판. 카테고리 필터 |
| `/tools` | 도구 — HTML 실험물 목록 (아직 비어 있음) |
| 유튜브 | 외부 링크 (꼰대레코드) |
| `/about` | 소개 — 메뉴에서 빠지고 푸터로 이동 |

- 사업자 정보는 `src/config.ts`의 `BUSINESS`에 모아두고 푸터에 상시 노출한다. **전자상거래법상 필수 표기라 지우면 안 된다.**
- 개편 전 URL(`/blog/*`, `/review`, `/club`)은 `public/_redirects`에서 301로 넘긴다. 색인이 죽지 않게 유지할 것.
...
3. **netlify 정리** — suengjaesmix.netlify.app은 /mix로 이전 완료. netlify 쪽 레포에
   `_redirects` 파일을 만들고 `/*  https://jaypd.kr/mix  301` 한 줄을 넣어야 옛 주소가 살아난다.
